...
// rendering and updation. Graphics graphics;
Image img;
Thread thread;
Snake snake; boolean gameOver; Token token; public void init() {
this.resize(400, 400);
...
this.addKeyListener(this); snake = new Snake(); token = new Token(snake); thread = new Thread(this); thread.start();
}
...
this specified color. */ graphics.setColor(Color.black); graphics.fillRect(0,0,400,400);
 if(!gameOver) { snake.draw(graphics); token.draw(graphics);
}
else {
graphics.setColor(Color.red);
graphics.drawString("Game Over", 180, 150); graphics.drawString("Score: " + token.getScore(), 180, 170);
}
...
public void run() {
for(;;) { if(!gameOver) {
snake.move(); this.checkGameOver(); token.snakeCollision();
} this.repaint(); try {
Thread.sleep(40); // to allow our eyes to pick up that the snake moved
} catch (InterruptedException ex) { ex.printStackTrace();
} }}
public void checkGameOver() { if(snake.getHeadX() < 0 || snake.getHeadX() > 396)
gameOver = true;
if(snake.getHeadY() < 0 || snake.getHeadY() > 396)
gameOver = true; if(snake.snakeCollision())
gameOver = true; }

public void keyPressed(KeyEvent kEvent) { if(!snake.isMoving()) {
if(kEvent.getKeyCode() == KeyEvent.VK_UP ||
kEvent.getKeyCode() == KeyEvent.VK_RIGHT || kEvent.getKeyCode() == KeyEvent.VK_DOWN) {
...
if(kEvent.getKeyCode() == KeyEvent.VK_DOWN) { if(snake.getYDir() != -1) {
snake.setYDir(1);
snake.setXDir(0); }
}
if(kEvent.getKeyCode() == KeyEvent.VK_LEFT) {
if(snake.getXDir() != 1) { snake.setXDir(-1); snake.setYDir(0);
}}
